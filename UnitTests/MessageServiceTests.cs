...
namespace UnitTests
{
	...
	public class MessageServiceTests
	{
		...
		public async Task GetChatDetails_Returns_Chat_Details_Correctly()
		{
			...
			Assert.AreEqual(expectedMessageCount, result.Messages.Count());
			Assert.AreEqual("Message 9", result.Messages.OrderByDescending(x=>x.CreatedOn).First().MessageContent);
		...
		public async Task LoadMessagesRange_Returns_Messages_Correctly()
		{
			...
			Assert.AreEqual("Message 19", result.First().MessageContent);
		}
		...
		public async Task CreateChatRoom_Creates_Chat_Room_Correctly()
		{
			...
			Assert.AreEqual(userId1, _dbContext.ChatRooms.First(x=>x.Id==chatRoomId).User1Id);
			Assert.AreEqual(userId2, _dbContext.ChatRooms.First(x => x.Id == chatRoomId).User2Id);			 
		}
	}
}
