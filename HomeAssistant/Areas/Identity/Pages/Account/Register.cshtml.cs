...
namespace HomeAssistant.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private HomeAssistantUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<HomeAssistantUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(HomeAssistantUser)}'. " +
                    $"Ensure that '{nameof(HomeAssistantUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<HomeAssistantUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<HomeAssistantUser>)_userStore;
        }
    }
}
